# Cursor/Claude Prompt - Criar/Implementar páginas de gerenciamento para   PAYMENT (RELACIONAR COM OUTROS DOMÍNIOS)   http://localhost:5173/v1 (@01_source/frontend) http://localhost:5174/v0 (@01_source/frontend_v0)
 
**Contexto:** `@02_docker/complete_schema_20260524_a.sql` e `@01_source/order_pickup_service` e `@01_source/backend/runtime`
...
2. Implementar/codar as páginas (novas ou atualizar) para gerenciar as tabelas relacionadas ao domínio     PAYMENT (RELACIONAR COM OUTROS DOMÍNIOS) 
3. Manter padrão UI
...
8. Endpoints: CRUD PAYMENT (RELACIONAR COM OUTROS DOMÍNIOS) (e outros associados com o projeto), webhook config, api key rotation entre outras possibilidades (que sejam necessárias)
...
1O. PROMPT
Codar o pedido em @docs/criar_paginas_payment.md. Ter como referência @02_docker/complete_schema_20260524_a.sql 

2o. PROMPT
Buscar criar mais páginas ou seções para  PAYMENT (RELACIONAR COM OUTROS DOMÍNIOS) . Se necessário criar tabelas e relações para termos nível profissional/mundial.

3O. PROMPT
Considerar InPost, DHL, Magalu, MercadoLivre, Amazon, DPD, Correios, CTT, WORTEN, EL CORTE INGLES entre outros players do segmento de LOCKERS em nível mundial para rever e para complementar PAYMENT (RELACIONAR COM OUTROS DOMÍNIOS) 
