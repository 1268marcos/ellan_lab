# Cursor/Claude Prompt - Criar/Implementar páginas de gerenciamento para    RENTAL   http://localhost:5173/v1 (@01_source/frontend) http://localhost:5174/v0 (@01_source/frontend_v0)
 
**Contexto:** `@02_docker/complete_schema_20260523_a.sql` e `@01_source/order_pickup_service` e `@01_source/backend/runtime`
...
2. Implementar/codar as páginas (novas ou atualizar) para gerenciar as tabelas relacionadas ao domínio  RENTAL
3. Manter padrão UI
...
8. Endpoints: CRUD RENTAL (e outros), webhook config, api key rotation entre outras possibilidades (que sejam necessárias)
