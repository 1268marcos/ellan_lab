...
| **Sprint 2 dominante + fatia S1** | Maximiza **progresso no gargalo** (Fiscal **~50%**, Contábil ~46%, consolidado ~58% vs limiares v2) **sem** abandonar o **#1 capacidade FE** (Sprint 1 ~**67%**; meta **≥60%** cumprida; ver **Metodo** *(x)*–*(xi)* e snapshots *(vii)*–*(x)*). |
...
- **Agora (com Fiscal + Contabil no mesmo sprint): ~58% no consolidado do sprint** *(sub-trilha Comprador ONLINE com evidência de checkout público: **~25%** — ver checklist acima; leitura lab **Metodo** *(xi)* **2026-04-30** + checkpoints gaps + emissores **2026-05-01**; comité pode recalcular macro se alterar pesos)*
...
| **Consolidado Sprint 2** (macro, todas as frentes do sprint) | **≥ 55%** | ~58% |
...
- **Consolidado Sprint 2 ajustado: ~58%** (D17–D18 com governanca de aceites + closeout assistido; regressão fiscal no host; cobertura pytest/Vitest na trilha financeira — ver **Metodo** *(xi)* + P0 gaps + P0 matriz emissores **2026-05-01**; escopo macro ainda inclui integracao ampliada)
...
| Sprint 2 | Dias 10-18 | **~58%** consolidado; Fiscal **~50%**; Contabil **~46%** | Trilha financeira D10-D18; OPS **~88%**, Suporte **~78%** no mesmo macro |
...
| Sprint 2 | **~58%** | `[~]` | **Prioridade `#1` negócio** (Fiscal + Contábil / D10–D18; Fiscal **~50%**, Contábil **~46%**); **coexiste** com S1 — throughput mínimo acordado |
...
- Atualizacao pos-D17 (governanca de aceites): consolidado **~50%**, Fiscal **22%**, Contabil **15%** (snapshot histórico; leitura atual no topo: **~58%** / **50%** / **46%** — **Metodo** *(xi)* + checkpoints 2026-05-01).
...
- Progresso Sprint 2 atualizado no topo deste documento: consolidado **~58%**, Fiscal **50%**, Contabil **46%** (ver **Metodo** *(xi)* e snapshot incremental Sprint 2).
...
Resumo:
- **Sprint 3** continua como camada de **confiabilidade** (CSP, TS, auditoria, SLO, incidente) **sem** ser tratada como “próximo saco de escopo” enquanto **Contábil (~46%)** da Sprint 2 não subir com evidência operacional (**Fiscal ~50%** no lab: limiar numérico v2 Fiscal cumprido; consolidado **~58%** vs **≥55%** — falta comprovação P0 / comité declarar PASS).
...
- **Sprint 2:** **~58%** consolidado (Fiscal **~50%**, Contábil **~46%**) — **foco comité até subir**.
...
- **Sprint 2:** consolidado **~52% → ~54% → ~56% → ~58%** (**Metodo** *(xi)*, **2026-04-30**–**2026-05-01**); **trilhas** com percentual explícito no doc: OPS **~88%**, Suporte **~78%**, Fiscal **~39% → ~48% → ~50%** (checkpoints **2026-05-01**, P0 gaps `SPRINT2_FISCAL_GAP_*` + matriz emissores `SPRINT2_FISCAL_ISSUER_GOVERNANCE_MATRIX_*`), Contábil **~46%** (checkpoints **2026-05-01** D14–D16 — **Metodo** *(xiv)*–*(xvi)*), Comprador ONLINE (evidência checkout; espelho **Metodo** *(x)* **~25%**), KIOSK operacional **0%**, Parceiros **0%**.
