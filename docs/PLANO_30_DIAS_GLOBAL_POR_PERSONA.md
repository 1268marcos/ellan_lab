...
- **Objetivo:** média dos 6 itens **~61%** (ver **Metodo** *(viii)*); próximo ganho típico: **E2E de erro 4xx** no checkout (order-pickup) ou CSS **kiosk/ops**. **Store e syncStatus: `[x]` 100%.** Catálogo→checkout + **POST** mock + «Processando…» no E2E: **`[x]`** (`e2e/public-catalog-to-checkout.spec.ts`). **Fatias 3–4** CSS (shell + painel fiscal + **`FiscalProfileForm` checkout**): **`[x]`**.  
...
- **Próxima trilha recomendada (solo):** Sprint 1 — E2E de falha no `POST` (4xx) no checkout ou início de migração CSS **kiosk/ops**; Sprint 2 — D14+ conforme sequência D10–D18.
