...
**Carimbo:** 2026-04-30 (percentuais Sprint 1 / trilhas; registo Lab alinhado a **Metodo** *(x)* e snapshot «trilhas C/E/F1/F2/D»; Sprint 2 financeiro **Metodo** *(xi)* — Fiscal **~33%**, Contábil **~20%**, consolidado **~54%**). **Atualização 2026-05-01 (lab):** Fiscal **~39%** (+6 p.p. vs snapshot *(xi)* ~33%): tranche 1 — evidência D10 export/ZIP assinado; tranche 2 — **handoff OPS** `SPRINT2_D10_PROVIDERS_OPS_HANDOFF_*` (`ellan_ops_fiscal_d10_handoff_v1`), espelho em **`fiscal/management-daily`**, ligação **D12/D13** (`d10_providers_ops_handoff`) e ZIP diário/executivo (ver **Evidência no repositório**). **Checkpoint 2026-05-01 (P0 Fiscal — gaps):** Fiscal **~48%** (+9 p.p. vs **~39%**): `GET /admin/fiscal/fiscal-gap-conciliation-snapshot`, `SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_*` no ZIP diário e `SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_EXEC_*` no ZIP executivo; `fiscal_gap_conciliation_snapshot_service.py` + **pytest** `test_fiscal_gap_conciliation_snapshot.py`; cartão em **`fiscal/management-daily`** (`fiscal_gap_conciliation_snapshot` no payload). **Consolidado ~56%** (+2 p.p. vs **~54%**). **Contábil ~46%** (+26 p.p. vs *(xi)* ~20%; **+8 p.p.** vs leitura **~38%** com D14): P0 **D14** — `GET /admin/fiscal/accounting/daily-operational-close` + **`SPRINT2_D14_ACCOUNTING_DAILY_OPERATIONAL_CLOSE_*`**; P0 **D15** — `GET /admin/fiscal/accounting/revenue-credits-delta` + **`SPRINT2_D15_REVENUE_CREDITS_DELTA_*`**; P0 **D16 (partners)** — `GET /admin/fiscal/accounting/partner-settlement-reconcile` + **`SPRINT2_D16_PARTNER_SETTLEMENT_RECONCILE_*`** nos ZIPs diário/executivo + pytest `test_accounting_daily_operational_close.py` + `test_accounting_revenue_credits_delta.py` + **`test_accounting_partner_settlement_reconcile.py`**. Rever esta secao sempre que o comité mudar prioridade ou quando Fiscal/Contabil/Consolidado S2 cruzarem limiares do **gate v2**.
...
| **Sprint 2 dominante + fatia S1** | Maximiza **progresso no gargalo** (Fiscal **~48%**, Contábil ~46%, consolidado ~56% vs limiares v2) **sem** abandonar o **#1 capacidade FE** (Sprint 1 ~**67%**; meta **≥60%** cumprida; ver **Metodo** *(x)*–*(xi)* e snapshots *(vii)*–*(x)*). |
...
| Produto / FE capacidade | **Sprint 1** | **`/ops/kiosk-touch-models`** (protótipos **`[~]` ~66%**), **checkout + kiosk OPS** (item migração **~25%** — fatias checkout 1–4 + mobile + **`opsKioskTouchModelsChrome.css`**), **E2E assistido** (**~20%** — A–D no Playwright + passo alocação Modelo D; falta fluxo físico completo no totem), **TS** **`[~]` ~93%**. |
...
- **~25–35%** em **Sprint 1** (protótipos KIOSK, E2E assistido, estilos checkout + kiosk OPS) para sustentar a média do checklist Sprint 1 (**≥60%**; leitura **2026-04-30 ~67%** — **Metodo** *(x)*) sem perder o desbloqueio de S3/S4.
...
As **trilhas** abaixo decompõem os **6 itens do checklist** em frentes de execução. A **média ~67%** do painel (**Metodo** *(x)*) vem só desses seis itens, não do checklist da Sprint 2.
...
| **E — E2E assistido** | Compra → … (jornadas públicas + OPS) | **~20%** | `[~]` | `public-catalog-to-checkout` (POST OK + **409**); cockpit: **A–D** → `/comprar`, `/checkout`, `/ops/pt/kiosk`, `/ops/dev/slots`; **Modelo D** — alocação SKU↔gaveta em `/ops/dev/slots` (`e2e/kiosk-touch-models.spec.ts`); **falta** fluxo físico KIOSK no totem (abertura → retirada/alocação real). |
...
**Leitura rápida:** trilhas **`[x]`** = **A** e **B**; gargalo de subida da média = **E** (fluxo físico ainda incompleto) + fecho qualitativo de **D** (utilizadores / visual) + resiliência checkout no item agregado; **C** e **F2** em bom rumo no indicador.
...
- [~] Eng/UX: validar fluxo KIOSK E2E assistido (compra -> pagamento -> abertura -> retirada/alocacao).
  - Indicador no painel: **~20%** — **OPS** cockpit com encadeamentos **A–D** (incl. `/ops/pt/kiosk`, `/ops/dev/slots`) + **`e2e/public-catalog-to-checkout.spec.ts`** (POST sucesso + **409**) + **`e2e/kiosk-touch-models.spec.ts`** (Modelo D: mocks `dev-admin`/`dev/catalog` + **POST** alocação por slot); ainda **sem** E2E do fluxo físico completo no totem (abertura → retirada/alocação). Ver também `e2e/checkout-dev-full.spec.ts` (token opcional).
...
- **Objetivo:** média dos 6 itens **~67%** (itens parciais: migração estilos **~25%**, E2E assistido **~20%**, TS **~93%**, protótipos **~66%**); próximo ganho típico: **E2E fluxo físico** KIOSK no totem ou **resiliência** checkout. **Store e syncStatus: `[x]` 100%.** Catálogo→checkout + **POST** mock + «Processando…» + **POST 4xx**: **`[x]`**. **CSS checkout** fatias 1–4 + mobile: **`[x]`**. **CSS kiosk/OPS** (cockpit touch): **incremento entregue** (`opsKioskTouchModelsChrome.css`).  
...
- **Onde codar em primeiro lugar (recomendação consolidada):** ver secção **«Recomendacao atual — onde codar»** acima do **Backlog por persona** — em síntese: **Sprint 2 dominante** (gate v2) + **fatia Sprint 1** para sustentar média **≥60%** dos seis itens (**~67%** em **2026-04-30** — **Metodo** *(x)*). **Foco Sprint 1:** trilhas **E** (físico), **D** (n≥8) e **checkout** resiliente.
...
- [~] Fiscal (ELLAN LAB + partners): operacao fiscal assistida + governanca de emissores e conformidade.
  - Progresso: **48%** (D10: **`fiscalD10ProvidersTracker`** + export **`SPRINT2_D10_PROVIDERS_TRACKER_*`** + **handoff OPS** `SPRINT2_D10_PROVIDERS_OPS_HANDOFF_*` + espelho em **`fiscal/management-daily`** + ligação **D12/D13** + anexos ZIP diário/executivo + Vitest; D11: **rollup por `order_id`** + export/ZIP diário e executivo + Vitest no util; **P0 gaps:** `GET /admin/fiscal/fiscal-gap-conciliation-snapshot` + **`SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_*`** / **`SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_EXEC_*`** + pytest `test_fiscal_gap_conciliation_snapshot.py`; regressão de resync no host; contrato fiscal padrão parceiros e aceite macro ainda em aberto)
- [~] Contabil (ELLAN LAB + partners): consolidacao contabil operacional + trilha de fechamento e evidencias.
  - Progresso: **46%** (aceite central D13-D18, exports, ZIP executivo; **pytest** `test_accounting_approvals_*` + **`test_accounting_revenue_credits_delta`** + **`test_accounting_partner_settlement_reconcile`** + **`test_accounting_daily_operational_close`**; **D14** + **D15** + **D16** rotas admin + **`SPRINT2_D14_*`** / **`SPRINT2_D15_*`** / **`SPRINT2_D16_PARTNER_SETTLEMENT_RECONCILE_*`** no daily/ZIP + contexto em **`FISCAL_ACCOUNTING_DAILY_APPROVAL`**; Vitest gate/D18/histórico/P01b; P0 **governança formal de provisões** e **100% parceiros prioritários** ainda em backlog)
...
- **Agora (com Fiscal + Contabil no mesmo sprint): ~56% no consolidado do sprint** *(sub-trilha Comprador ONLINE com evidência de checkout público: **~25%** — ver checklist acima; leitura lab **Metodo** *(xi)* **2026-04-30** + checkpoint gaps **2026-05-01**; comité pode recalcular macro se alterar pesos)*
...
| --- | ---: | --- |
| **Fiscal** (ELLAN LAB + partners) | **≥ 50%** | ~48% |
| **Contábil** (ELLAN LAB + partners) | **≥ 40%** | ~46% |
| **Consolidado Sprint 2** (macro, todas as frentes do sprint) | **≥ 55%** | ~56% |
...
- **Consolidado Sprint 2 ajustado: ~56%** (D17–D18 com governanca de aceites + closeout assistido; regressão fiscal no host; cobertura pytest/Vitest na trilha financeira — ver **Metodo** *(xi)* + P0 gaps **2026-05-01**; escopo macro ainda inclui integracao ampliada)
- **Fiscal (ELLAN LAB + partners): 48%** (D10 util + export **`SPRINT2_D10_PROVIDERS_TRACKER_*`** + handoff **`SPRINT2_D10_PROVIDERS_OPS_HANDOFF_*`** no ZIP diário/executivo + espelho management-daily + D12/D13; cockpits; endpoints admin fiscais + trilha D11 com evidência **rollup `order_id`** anexável ao daily/ZIP + **P0** `SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_*` / `*_EXEC_*` + regressão de resync no host)
- **Contabil (ELLAN LAB + partners): 46%** (trilha D13-D18; D14 snapshot diário rev.rec./KPI/ledger/ciclos + **pytest** `test_accounting_daily_operational_close`; D15 + D16 + **pytest** `test_accounting_revenue_credits_delta` + **`test_accounting_partner_settlement_reconcile`**; API list/compare/post/latest + retenção/divergência prolongada com **pytest** dedicado; FE gate/D18/histórico/P01b)
...
| Sprint 2 | Dias 10-18 | **~56%** consolidado; Fiscal **~48%**; Contabil **~46%** | Trilha financeira D10-D18; OPS **~88%**, Suporte **~78%** no mesmo macro |
...
| Sprint 2 | **~56%** | `[~]` | **Prioridade `#1` negócio** (Fiscal + Contábil / D10–D18; Fiscal **~48%**, Contábil **~46%**); **coexiste** com S1 — throughput mínimo acordado |
...
**Leitura com E2E ~20%:** cálculo bruto **(25+100+100+93+66+20)/6 ≈ 67,3%** — painel executivo mantém **`~67%`** por arredondamento (**Metodo** *(xiii)*).

**Evidência:** commits `style(ops): trilha F2`, `style(checkout): trilha F1`, `feat(ops): trilha D`, `test(e2e): trilha E` (ext. A–D), `refactor(ts): trilha C` (`OpsScenarioPresets`, `OpsHelpTutorialModal`); `npm run typecheck`, `typecheck:strict-core`, `build`; Playwright `e2e/kiosk-touch-models.spec.ts` verde (incl. **Modelo D** — alocação por slot, **Metodo** *(xiii)*).
...
- [~] Completar auditoria ponta a ponta em fluxos de alto impacto.
  - Progresso: **58%** (trilha E2E + rollup por parceiro em `fiscal/sprint3-partner-audit` + export slice + **espelho ZIP** `SPRINT3_PARTNER_AUDIT_MIRROR_ATTACH` em `fiscal/management-daily` / `fiscal/accounting-close` via `fiscalSprint3PartnerAuditMirror.js`; checklist de handoff (6 itens) + export JSON; **faixa Sprint 3** no hub `fiscal/global`; falta reconciliação **real** multi-parceiro e evidência de utilizadores nos dailies)
...
- Atualizacao pos-D17 (governanca de aceites): consolidado **~50%**, Fiscal **22%**, Contabil **15%** (snapshot histórico; leitura atual no topo: **~56%** / **48%** / **46%** — **Metodo** *(xi)* + checkpoints 2026-05-01).
...
- Progresso Sprint 2 atualizado no topo deste documento: consolidado **~56%**, Fiscal **48%**, Contabil **46%** (ver **Metodo** *(xi)* e snapshot incremental Sprint 2).
...
Resumo:
- **Sprint 3** continua como camada de **confiabilidade** (CSP, TS, auditoria, SLO, incidente) **sem** ser tratada como “próximo saco de escopo” enquanto **Fiscal (~48%)** e **Contábil (~46%)** da Sprint 2 não subirem com evidência operacional (Fiscal ainda abaixo de **50%** no gate v2).
...
- **Sprint 2:** **~56%** consolidado (Fiscal **~48%**, Contábil **~46%**) — **foco comité até subir**.
...
- [~] **P0-1 Auditoria ponta a ponta dos fluxos críticos (pedido -> emissão -> reconciliação -> handoff).**  
  ...
  - Progresso atual: **58%** (alinhado ao item «auditoria ponta a ponta» do checklist Sprint 3 neste `.md`, incl. espelho ZIP `SPRINT3_PARTNER_AUDIT_MIRROR_ATTACH`).
...
- **Sprint 3:** **~68%** (item auditoria **58%**; média das seis frentes — secção Sprint 3 e **Metodo** *(xii)* no painel percentual)
- **Sprint 4:** **~32%**
...
- **Sprint 2:** consolidado **~52% → ~54% → ~56%** (**Metodo** *(xi)*, **2026-04-30**–**2026-05-01**); **trilhas** com percentual explícito no doc: OPS **~88%**, Suporte **~78%**, Fiscal **~39% → ~48%** (checkpoints **2026-05-01**, P0 gaps `SPRINT2_FISCAL_GAP_*`), Contábil **~46%** (checkpoints **2026-05-01** D14–D16 — **Metodo** *(xiv)*–*(xvi)*), Comprador ONLINE (evidência checkout; espelho **Metodo** *(x)* **~25%**), KIOSK operacional **0%**, Parceiros **0%**.
