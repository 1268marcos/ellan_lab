...
**Carimbo:** 2026-04-30 (percentuais Sprint 1 / trilhas; registo Lab alinhado a **Metodo** *(x)* e snapshot «trilhas C/E/F1/F2/D»; Sprint 2 financeiro **Metodo** *(xi)* — Fiscal **~33%**, Contábil **~20%**, consolidado **~54%**). **Atualização 2026-05-01 (lab):** Fiscal **~39%** (+6 p.p. vs snapshot *(xi)* ~33%): tranche 1 — evidência D10 export/ZIP assinado; tranche 2 — **handoff OPS** `SPRINT2_D10_PROVIDERS_OPS_HANDOFF_*` (`ellan_ops_fiscal_d10_handoff_v1`), espelho em **`fiscal/management-daily`**, ligação **D12/D13** (`d10_providers_ops_handoff`) e ZIP diário/executivo (ver **Evidência no repositório**). **Contábil ~30%** (+10 p.p. vs *(xi)* ~20%): P0 **D15** — `GET /admin/fiscal/accounting/revenue-credits-delta` + **`SPRINT2_D15_REVENUE_CREDITS_DELTA_*`** no pacote diário/executivo + pytest `test_accounting_revenue_credits_delta.py`. Rever esta secao sempre que o comité mudar prioridade ou quando Fiscal/Contabil/Consolidado S2 cruzarem limiares do **gate v2**.
...
| **Sprint 2 dominante + fatia S1** | Maximiza **progresso no gargalo** (Fiscal **~39%**, Contábil ~30%, consolidado ~54% vs limiares v2) **sem** abandonar o **#1 capacidade FE** (Sprint 1 ~**67%**; meta **≥60%** cumprida; ver **Metodo** *(x)*–*(xi)* e snapshots *(vii)*–*(x)*). |
...
- [~] Contabil (ELLAN LAB + partners): consolidacao contabil operacional + trilha de fechamento e evidencias.
  - Progresso: **30%** (aceite central D13-D18, exports, ZIP executivo; **pytest** `test_accounting_approvals_*` + **`test_accounting_revenue_credits_delta`**; **D15** `GET /admin/fiscal/accounting/revenue-credits-delta` + **`SPRINT2_D15_REVENUE_CREDITS_DELTA_*`** no daily/ZIP + contexto em **`FISCAL_ACCOUNTING_DAILY_APPROVAL`**; Vitest gate/D18/histórico/P01b; P0 reconciliação **repasse por parceiro** ainda majoritariamente em backlog)
...
| **Contábil** (ELLAN LAB + partners) | **≥ 40%** | ~30% |
...
- **Contabil (ELLAN LAB + partners): 30%** (trilha D13-D18; D15 delta receita/estornos/créditos + **pytest** `test_accounting_revenue_credits_delta`; API list/compare/post/latest + retenção/divergência prolongada com **pytest** dedicado; FE gate/D18/histórico/P01b)
...
| Sprint 2 | Dias 10-18 | **~54%** consolidado; Fiscal **~39%**; Contabil **~30%** | Trilha financeira D10-D18; OPS **~88%**, Suporte **~78%** no mesmo macro |
...
| Sprint 2 | **~54%** | `[~]` | **Prioridade `#1` negócio** (Fiscal + Contábil / D10–D18; Fiscal **~39%**, Contábil **~30%**); **coexiste** com S1 — throughput mínimo acordado |
...
- Atualizacao pos-D17 (governanca de aceites): consolidado **~50%**, Fiscal **22%**, Contabil **15%** (snapshot histórico; leitura atual no topo: **~54%** / **39%** / **30%** — **Metodo** *(xi)* + checkpoints 2026-05-01).
...
- Progresso Sprint 2 atualizado no topo deste documento: consolidado **~54%**, Fiscal **39%**, Contabil **30%** (ver **Metodo** *(xi)* e snapshot incremental Sprint 2).
...
Resumo:
- **Sprint 3** continua como camada de **confiabilidade** (CSP, TS, auditoria, SLO, incidente) **sem** ser tratada como “próximo saco de escopo” enquanto **Fiscal (~39%)** e **Contábil (~30%)** da Sprint 2 não subirem com evidência operacional.
...
- **Sprint 2:** **~54%** consolidado (Fiscal **~39%**, Contábil **~30%**) — **foco comité até subir**.
...
- **Sprint 2:** consolidado **~52% → ~54%** (**Metodo** *(xi)*, **2026-04-30**); **trilhas** com percentual explícito no doc: OPS **~88%**, Suporte **~78%**, Fiscal **~39%** (checkpoints **2026-05-01**), Contábil **~30%** (checkpoint **2026-05-01** D15 — **Metodo** *(xiv)*), Comprador ONLINE (evidência checkout; espelho **Metodo** *(x)* **~25%**), KIOSK operacional **0%**, Parceiros **0%**.
