...
**Carimbo:** 2026-04-30 (percentuais Sprint 1 / trilhas; registo Lab alinhado a **Metodo** *(x)* e snapshot «trilhas C/E/F1/F2/D»; Sprint 2 financeiro **Metodo** *(xi)* — Fiscal **~33%**, Contábil **~20%**, consolidado **~54%**). **Atualização 2026-05-01 (lab):** Fiscal **~39%** (+6 p.p. vs snapshot *(xi)* ~33%): tranche 1 — evidência D10 export/ZIP assinado; tranche 2 — **handoff OPS** `SPRINT2_D10_PROVIDERS_OPS_HANDOFF_*` (`ellan_ops_fiscal_d10_handoff_v1`), espelho em **`fiscal/management-daily`**, ligação **D12/D13** (`d10_providers_ops_handoff`) e ZIP diário/executivo (ver **Evidência no repositório**). **Contábil ~38%** (+18 p.p. vs *(xi)* ~20%; **+8 p.p.** vs leitura **~30%** com D16): P0 **D15** — `GET /admin/fiscal/accounting/revenue-credits-delta` + **`SPRINT2_D15_REVENUE_CREDITS_DELTA_*`**; P0 **D16 (partners)** — `GET /admin/fiscal/accounting/partner-settlement-reconcile` + **`SPRINT2_D16_PARTNER_SETTLEMENT_RECONCILE_*`** nos ZIPs diário/executivo + pytest `test_accounting_revenue_credits_delta.py` + **`test_accounting_partner_settlement_reconcile.py`**. Rever esta secao sempre que o comité mudar prioridade ou quando Fiscal/Contabil/Consolidado S2 cruzarem limiares do **gate v2**.
...
| **Sprint 2 dominante + fatia S1** | Maximiza **progresso no gargalo** (Fiscal **~39%**, Contábil ~38%, consolidado ~54% vs limiares v2) **sem** abandonar o **#1 capacidade FE** (Sprint 1 ~**67%**; meta **≥60%** cumprida; ver **Metodo** *(x)*–*(xi)* e snapshots *(vii)*–*(x)*). |
...
- [~] Contabil (ELLAN LAB + partners): consolidacao contabil operacional + trilha de fechamento e evidencias.
  - Progresso: **38%** (aceite central D13-D18, exports, ZIP executivo; **pytest** `test_accounting_approvals_*` + **`test_accounting_revenue_credits_delta`** + **`test_accounting_partner_settlement_reconcile`**; **D15** + **D16** `GET /admin/fiscal/accounting/revenue-credits-delta` / **`partner-settlement-reconcile`** + **`SPRINT2_D15_*`** / **`SPRINT2_D16_PARTNER_SETTLEMENT_RECONCILE_*`** no daily/ZIP + contexto em **`FISCAL_ACCOUNTING_DAILY_APPROVAL`**; Vitest gate/D18/histórico/P01b; P0 **provisões / repasse operacional completo** e **100% parceiros prioritários** ainda em backlog)
...
- **Agora (com Fiscal + Contabil no mesmo sprint): ~54% no consolidado do sprint** *(sub-trilha Comprador ONLINE com evidência de checkout público: **~25%** — ver checklist acima; leitura lab **Metodo** *(xi)* **2026-04-30**; comité pode recalcular macro se alterar pesos)*
...
| **Contábil** (ELLAN LAB + partners) | **≥ 40%** | ~38% |
...
| Sprint 2 | Dias 10-18 | **~54%** consolidado; Fiscal **~39%**; Contabil **~38%** | Trilha financeira D10-D18; OPS **~88%**, Suporte **~78%** no mesmo macro |
...
| Sprint 2 | **~54%** | `[~]` | **Prioridade `#1` negócio** (Fiscal + Contábil / D10–D18; Fiscal **~39%**, Contábil **~38%**); **coexiste** com S1 — throughput mínimo acordado |
...
- Atualizacao pos-D17 (governanca de aceites): consolidado **~50%**, Fiscal **22%**, Contabil **15%** (snapshot histórico; leitura atual no topo: **~54%** / **39%** / **38%** — **Metodo** *(xi)* + checkpoints 2026-05-01).
...
- Progresso Sprint 2 atualizado no topo deste documento: consolidado **~54%**, Fiscal **39%**, Contabil **38%** (ver **Metodo** *(xi)* e snapshot incremental Sprint 2).
...
Resumo:
- **Sprint 3** continua como camada de **confiabilidade** (CSP, TS, auditoria, SLO, incidente) **sem** ser tratada como “próximo saco de escopo” enquanto **Fiscal (~39%)** e **Contábil (~38%)** da Sprint 2 não subirem com evidência operacional.
...
- **Sprint 2:** **~54%** consolidado (Fiscal **~39%**, Contábil **~38%**) — **foco comité até subir**.
...
- **Sprint 2:** consolidado **~52% → ~54%** (**Metodo** *(xi)*, **2026-04-30**); **trilhas** com percentual explícito no doc: OPS **~88%**, Suporte **~78%**, Fiscal **~39%** (checkpoints **2026-05-01**), Contábil **~38%** (checkpoints **2026-05-01** D15 — **Metodo** *(xiv)* + D16 partners — **Metodo** *(xv)*), Comprador ONLINE (evidência checkout; espelho **Metodo** *(x)* **~25%**), KIOSK operacional **0%**, Parceiros **0%**.
