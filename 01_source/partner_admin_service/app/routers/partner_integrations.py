...
from app.services import api_key_service, contact_service, webhook_service
...
@router.get("/{partner_id}/contacts", response_model=PartnerContactListOut)
def list_contacts(
    partner_id: str,
    partner_type: str = Query(...),
    db: Session = Depends(get_db),
) -> PartnerContactListOut:
    return contact_service.list_contacts(db, partner_id, partner_type)


@router.post("/{partner_id}/contacts", response_model=PartnerContactOut, status_code=status.HTTP_201_CREATED)
def create_contact(
    partner_id: str,
    ...
    return contact_service.create_contact(db, partner_id, partner_type, body)
