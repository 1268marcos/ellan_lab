-- Ecossistema mundial + auditoria + entregas webhook (capability admin)
...
CREATE INDEX IF NOT EXISTS ix_cap_eco_player_segment ON capability_ecosystem_player(segment_id, is_active);
...
CREATE INDEX IF NOT EXISTS ix_cap_profile_player_binding_profile ON capability_profile_player_binding(profile_id);
...
CREATE INDEX IF NOT EXISTS ix_cap_profile_audit_profile ON capability_profile_audit_log(profile_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS ix_cap_profile_wh_del_webhook ON capability_profile_webhook_delivery(webhook_id, created_at DESC);
