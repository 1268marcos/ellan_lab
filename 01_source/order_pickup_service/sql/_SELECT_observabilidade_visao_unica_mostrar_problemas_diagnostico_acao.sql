-- acao_recomendada já calculada.
-- Ela aponta, por pedido:
--  semáforos
--  diagnóstico final
...
--  Leitura das ações:
-- 
-- REPROCESSAR_DEADLINE
-- 
-- deadline PICKUP_TIMEOUT falhou
-- pedido ainda está em PAID_PENDING_PICKUP
-- o fluxo pode ser retomado pelo lifecycle
-- 
-- LIBERAR_RUNTIME
--
-- o prazo já venceu
-- pedido segue ativo
-- allocation ainda não foi liberada
-- normalmente indica que banco/runtime ficaram presos
--
-- GERAR_CREDITO
--
-- pedido já deveria estar em EXPIRED_CREDIT_50
-- mas não existe crédito registrado
--
-- REVISAR_FLUXO_POS_PAGAMENTO
-- 
-- caso clássico que apareceu nos seus dumps: pedido pago, 
-- pickup ativo, mas allocation ainda em RESERVED_PENDING_PAYMENT 
-- em vez de estado pós-pagamento
--
-- REVISAR_CANCELAMENTO_PREPAYMENT
--
-- o PREPAYMENT_TIMEOUT falhou mesmo com pagamento aprovado
-- isso é um cheiro forte de inconsistência do fluxo de cancelamento do deadline
...
SELECT
    ...
    CASE
        WHEN b.payment_status = 'APPROVED'
         ...
         AND COALESCE(b.pickup_expires_at, b.pickup_deadline_at) IS NOT NULL
         AND now() > COALESCE(b.pickup_expires_at, b.pickup_deadline_at)
         AND b.allocation_state <> 'RELEASED'
        ...
        WHEN b.payment_status = 'APPROVED'
         AND b.order_status = 'PAID_PENDING_PICKUP'
         AND b.allocation_state = 'RESERVED_PENDING_PAYMENT'
        ...
        WHEN b.order_status IN ('PICKED_UP', 'DISPENSED')
        THEN 'OK_RETIRADO'
    ...
    CASE
        ...
        WHEN b.payment_status = 'APPROVED'
         AND b.order_status = 'PAID_PENDING_PICKUP'
         AND b.allocation_state = 'RESERVED_PENDING_PAYMENT'
         AND COALESCE(b.pickup_deadline_status, '') IN ('PENDING', 'EXECUTED', 'CANCELLED')
        ...
        WHEN b.payment_status = 'APPROVED'
         AND b.order_status = 'PAID_PENDING_PICKUP'
         AND COALESCE(b.pickup_expires_at, b.pickup_deadline_at) > now()
        THEN 'ACOMPANHAR'
...
ORDER BY
    CASE
        WHEN
            COALESCE(b.pickup_deadline_status, '') = 'FAILED'
            OR (
            ...
            OR (
                b.order_status = 'EXPIRED_CREDIT_50'
                AND b.credit_id IS NULL
            )
        THEN 0
        WHEN b.payment_status = 'APPROVED' AND b.order_status = 'PAID_PENDING_PICKUP'
        THEN 1
        ELSE 2
    END,
    b.created_at DESC;
