...
def seed_product_categories(db: Session):
    ...
    for cat in categories:
        ...
        if not existing:
            db.add(cat)
            logger.info(f"Categoria adicionada: {cat.id} - {cat.name}")
    
    db.commit()
    logger.info(f"Seed concluído: {len([c for c in categories if not db.query(ProductCategory).filter(ProductCategory.id == c.id).first()])} novas categorias inseridas")
