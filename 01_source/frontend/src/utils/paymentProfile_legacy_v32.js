...
export const PAYMENT_METHODS = {
  ...
  MULTIBANCO_REFERENCE: {
    ...
    gatewayMethod: "multibanco_reference",
    kioskInterface: "qr_code",
    onlineInterface: "web_token",
    requiresPhone: false,
    walletProvider: null,
  ...
  MERCADO_PAGO_WALLET: {
    ...
    gatewayMethod: "mercado_pago_wallet",
    kioskInterface: "qr_code",
    onlineInterface: "web_token",
    requiresPhone: false,
...
export function mapGatewayMethod(uiMethod) {
  return getPaymentProfile(uiMethod)?.gatewayMethod || String(uiMethod || "").trim();
}
...
export function buildGatewayPaymentPayload({
  ...
  const gatewayMethod = mapGatewayMethod(uiMethod);
  const paymentInterface = mapPaymentInterface(uiMethod, "kiosk");
  const walletProvider = mapWalletProvider(uiMethod);
