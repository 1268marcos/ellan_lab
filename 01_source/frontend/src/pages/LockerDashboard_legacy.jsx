...
/**
 * helper para formatar valor/data
 */
...
export default function LockerDashboard({ region = "PT" }) {
  ...
  async function fetchOrdersOnce() {
    ...
    try {
      ...
      if (ordersFilterStatus) {
        params.set("status", ordersFilterStatus);
      }
  ...
  // recarregar quando mudar filtro de status para os pedidos (orders)
  useEffect(() => {
    fetchOrdersOnce();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [region, ordersFilterStatus]);
  ...
  async function regenerateManualCode() {
    if (!currentOrder?.order_id) {
      setPayResp("❌ Nenhum pedido selecionado para regenerar código.");
      return;
    }

    if (currentOrder?.status !== "PAID_PENDING_PICKUP") {
      setPayResp("❌ Só é possível regenerar código para pedido em PAID_PENDING_PICKUP.");
      return;
    }
    ...
    try {
      ...
      const text = await res.text();
      if (!res.ok) {
        setPayResp(`❌ Falha ao regenerar código manual\nHTTP ${res.status}: ${text}`);
        return;
      }
      ...
      setPayResp(`❌ Erro ao regenerar código manual\n${String(e?.message || e)}`);
    } finally {
  ...
  async function simulatePayment() {
    ...
    if (currentOrder?.status === "PAID_PENDING_PICKUP") {
      setPayResp("⚠️ Pedido já pago. Use “Gerar/Atualizar” ou “Regenerar código manual do pedido selecionado”.");
      return;
    }
        ...
        <div style={{ borderRadius: 14, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)", padding: 14, display: "grid", gap: 12, alignContent: "start" }}>
          ...
          <div style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(0,0,0,0.18)", padding: 12, display: "grid", gap: 10 }}>
            ...
            {currentOrder?.status === "PAID_PENDING_PICKUP" ? (
              ...
              >
                ⚠️ Este pedido já está pago e aguardando retirada.  
                Para recuperar o código manual, use <b>“Gerar/Atualizar”</b> no painel de retirada
                ou o botão <b>“Regenerar código manual do pedido selecionado”</b>.
              </div>
            ) : null}
            ...
            >
              {regenCodeLoading
                ? "Regenerando código..."
                : "Regenerar código manual do pedido selecionado"}
            </button>
