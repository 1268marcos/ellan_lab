...
export default function FiscalGlobalPage() {
  ...
  async function loadGlobalFiscalData() {
    ...
    setError("");
    try {
      const [catalogRes, matrixRes, adaptersRes, fixturesRes] = await Promise.all([
      ...
      const [catalogPayload, matrixPayload, adaptersPayload, fixturesPayload] = await Promise.all([
        catalogRes.json().catch(() => ({})),
        matrixRes.json().catch(() => ({})),
        adaptersRes.json().catch(() => ({})),
        fixturesRes.json().catch(() => ({})),
      ]);
      if (!catalogRes.ok || !matrixRes.ok || !adaptersRes.ok || !fixturesRes.ok) {
        const detail = String(
          catalogPayload?.detail ||
            matrixPayload?.detail ||
            adaptersPayload?.detail ||
            fixturesPayload?.detail ||
            "Falha ao carregar catálogo/matriz fiscal global."
        );
