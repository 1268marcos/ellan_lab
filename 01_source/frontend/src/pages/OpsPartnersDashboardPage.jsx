        ...
        {payload ? (
          <>
            <div style={kpiGridStyle}>
              <KpiCard label="Total eventos" value={payload?.kpis?.total_events ?? "-"} />
              <KpiCard label="Erro %" value={payload?.kpis ? `${payload.kpis.error_rate_pct}%` : "-"} />
              <KpiCard label="Delta total %" value={payload?.compare ? `${payload.compare.total_delta_pct}%` : "-"} />
              <KpiCard label="Confianca" value={payload?.compare?.confidence_level ?? "-"} />
            </div>
...
function KpiCard({ label, value }) {
  return (
    <article style={kpiCardStyle}>
      <small style={{ color: "#94A3B8", textTransform: "uppercase", fontSize: 11 }}>{label}</small>
