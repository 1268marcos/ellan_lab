...
def test_maybe_mtls_middleware_path_missing(monkeypatch):
...
def test_maybe_mtls_loader_none(monkeypatch):
    import app.main as mm
    ...
    monkeypatch.setattr(mm.importlib.util, "spec_from_file_location", lambda *a, **k: Spec())
    importlib.reload(mm)
    monkeypatch.setenv("MTLS_ENFORCE", "0")
    importlib.reload(mm)
