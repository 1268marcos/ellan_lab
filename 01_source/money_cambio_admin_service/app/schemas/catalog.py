...
class WalletProviderCatalogIn(BaseModel):
    code: str
    name: str
    metadata_json: dict[str, Any] = Field(default_factory=dict)
    is_active: bool = True
...
class WalletProviderCatalogUpdate(BaseModel):
    name: str | None = None
    metadata_json: dict[str, Any] | None = None
    is_active: bool | None = None


class WalletProviderCatalogOut(BaseModel):
    model_config = ConfigDict(from_attributes=True)
...
class WalletProviderCatalogListOut(BaseModel):
    items: list[WalletProviderCatalogOut]
    total: int
