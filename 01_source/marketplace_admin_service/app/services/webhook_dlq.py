...
def apply_delivery_status(db: Session, *, webhook_id: str, success: bool, response_snippet: str | None) -> str:
    ...
    from app.models.marketplace_alerts_webhooks import MarketplaceCapabilityWebhookDelivery

    fail_count = (
        db.query(MarketplaceCapabilityWebhookDelivery)
        .filter(
            MarketplaceCapabilityWebhookDelivery.webhook_id == webhook_id,
            MarketplaceCapabilityWebhookDelivery.status == STATUS_FAILED,
        )
        .count()
    )
