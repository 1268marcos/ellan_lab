...
def apply_delivery_status(db: Session, *, webhook_id: str, success: bool, response_snippet: str | None) -> str:
    ...
    from app.models.professional_ops import HardwareCapabilityWebhookDelivery

    fail_count = (
        db.query(HardwareCapabilityWebhookDelivery)
        .filter(
            HardwareCapabilityWebhookDelivery.webhook_id == webhook_id,
            HardwareCapabilityWebhookDelivery.status == STATUS_FAILED,
        )
        .count()
    )
