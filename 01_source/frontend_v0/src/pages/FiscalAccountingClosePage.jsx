...
const PAGE_VERSION = "fiscal/accounting-close v1.2.14-partner-provisions-governance-exec";
...
export default function FiscalAccountingClosePage() {
  ...
  async function exportZip() {
    ...
    try {
      ...
      const signedSprint4KioskTouch = await buildSignedPayload(
        buildSprint4KioskTouchUatModelsPayload(nowIso, storedMatrix && typeof storedMatrix === "object" ? storedMatrix : {})
      );
      zipEntries[`${DAILY_AUDIT_PREFIX}_${day}_SPRINT4_KIOSK_TOUCH_UAT_MODELS_A_D_${ts}.json`] = strToU8(
        JSON.stringify(signedSprint4KioskTouch, null, 2)
      );
    } catch {
      try {
    ...
    try {
      if (INTERNAL_TOKEN) {
        const r14 = await fetch(
          `${BILLING_BASE}/admin/fiscal/accounting/daily-operational-close?${new URLSearchParams({
            date: String(nowIso).slice(0, 10),
          }).toString()}`,
          { method: "GET", headers: headersJson() },
        );
        const data14 = await r14.json().catch(() => ({}));
        if (r14.ok && data14?.ok && data14.scope === "SPRINT2_D14_ACCOUNTING_DAILY_OPERATIONAL_CLOSE") {
          const { ok: _ok14, ...rest14 } = data14;
          const signedD14Exec = await buildSignedPayload(rest14);
          zipEntries[`${DAILY_AUDIT_PREFIX}_${day}_SPRINT2_D14_ACCOUNTING_DAILY_OPERATIONAL_CLOSE_EXEC_${ts}.json`] = strToU8(
            JSON.stringify(signedD14Exec, null, 2),
          );
        }
      }
    } catch {
      // D14 executivo opcional
    }
    try {
      if (INTERNAL_TOKEN) {
        const r = await fetch(
          `${BILLING_BASE}/admin/fiscal/accounting/revenue-credits-delta?${new URLSearchParams({
            date: String(nowIso).slice(0, 10),
          }).toString()}`,
          { method: "GET", headers: headersJson() },
        );
        const data = await r.json().catch(() => ({}));
        if (r.ok && data?.ok && data.scope === "SPRINT2_D15_REVENUE_CREDITS_DELTA") {
          const { ok: _ok, ...rest } = data;
          const signedD15Exec = await buildSignedPayload(rest);
          zipEntries[`${DAILY_AUDIT_PREFIX}_${day}_SPRINT2_D15_REVENUE_CREDITS_DELTA_EXEC_${ts}.json`] = strToU8(
            JSON.stringify(signedD15Exec, null, 2),
          );
        }
      }
    } catch {
    ...
    try {
      if (INTERNAL_TOKEN) {
        const rPv = await fetch(
          `${BILLING_BASE}/admin/fiscal/accounting/partner-provisions-governance?${new URLSearchParams({
            date: String(nowIso).slice(0, 10),
            partner_limit: "200",
          }).toString()}`,
          { method: "GET", headers: headersJson() },
        );
        const dataPv = await rPv.json().catch(() => ({}));
        if (rPv.ok && dataPv?.ok && dataPv.scope === "SPRINT2_PARTNER_PROVISIONS_GOVERNANCE") {
          const { ok: _okPv, ...restPv } = dataPv;
          const signedPvExec = await buildSignedPayload(restPv);
          zipEntries[`${DAILY_AUDIT_PREFIX}_${day}_SPRINT2_PARTNER_PROVISIONS_GOVERNANCE_EXEC_${ts}.json`] = strToU8(
            JSON.stringify(signedPvExec, null, 2),
          );
        }
      }
    } catch {
      // P0 provisões parceiros executivo opcional
    }
    try {
      if (INTERNAL_TOKEN) {
        const rFg = await fetch(
          `${BILLING_BASE}/admin/fiscal/fiscal-gap-conciliation-snapshot?${new URLSearchParams({
            date: String(nowIso).slice(0, 10),
            refresh: "false",
          }).toString()}`,
          { method: "GET", headers: headersJson() },
        );
        const dataFg = await rFg.json().catch(() => ({}));
        if (rFg.ok && dataFg?.ok && dataFg.scope === "SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT") {
          const { ok: _okFg, ...restFg } = dataFg;
          const signedFgExec = await buildSignedPayload(restFg);
          zipEntries[`${DAILY_AUDIT_PREFIX}_${day}_SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_EXEC_${ts}.json`] = strToU8(
            JSON.stringify(signedFgExec, null, 2),
          );
        }
      }
    } catch {
    ...
    setStatus(
      "Pacote ZIP exportado: close + gate + aprovação + D16 aprovações + P0-1b (com token) + D10 tracker + D10 OPS handoff + P0 gaps fiscal (`SPRINT2_FISCAL_GAP_CONCILIATION_SNAPSHOT_EXEC_*`) + matriz emissores (`SPRINT2_FISCAL_ISSUER_GOVERNANCE_MATRIX_EXEC_*`) + provisões parceiros (`SPRINT2_PARTNER_PROVISIONS_GOVERNANCE_EXEC_*`) + D14 fechamento diário (`SPRINT2_D14_ACCOUNTING_DAILY_OPERATIONAL_CLOSE_EXEC_*`) + D15 receita/estornos (`SPRINT2_D15_REVENUE_CREDITS_DELTA_EXEC_*`) + D16 repasse parceiro (`SPRINT2_D16_PARTNER_SETTLEMENT_RECONCILE_EXEC_*`) + D11 rollup + D12/D13 executivos + D18 + matriz Sprint 4 + resumo Go/No-Go + pilotos + carimbo P0-3 + espelhos gate v2 e Sprint 3 partner-audit quando houver no browser.",
    );
    window.setTimeout(() => setStatus(""), 2200);
  }
