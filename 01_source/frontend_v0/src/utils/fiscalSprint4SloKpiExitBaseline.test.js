...
import {
  ...
  SPRINT4_SLO_CALIBRATION_BR_PT_EVIDENCE,
...
describe("fiscalSprint4SloKpiExitBaseline", () => {
  it("expõe baseline numérico por persona (Sprint 4)", () => {
    expect(SPRINT4_SLO_KPI_EXIT_BASELINE_VERSION).toMatch(/sprint4-slo-kpi-exit-baseline-v2/);
  ...
  it("documenta config mínima produção slo-alerts e comandos de coleta", () => {
    ...
    expect(SPRINT4_SLO_METRICS_COLLECT_COMMANDS.some((l) => l.includes("divergence-health"))).toBe(true);
    expect(SPRINT4_SLO_METRICS_COLLECT_COMMANDS.some((l) => l.includes("accounting-approvals"))).toBe(true);
    expect(SPRINT4_SLO_CALIBRATION_BR_PT_EVIDENCE.scope).toMatch(/SPRINT4_SLO_CALIBRATION/);
  });
});
