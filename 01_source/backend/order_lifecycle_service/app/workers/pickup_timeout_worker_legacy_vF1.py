...
from app.services.deadline_engine import execute_pickup_timeout
...
def claim_due_deadline_ids(db: Session, batch_size: int = 100) -> list[UUID]:
    stmt = (
        ...
        .where(LifecycleDeadline.deadline_type == DeadlineType.PICKUP_TIMEOUT)
    ...
    for deadline_id in deadline_ids:
        try:
            with SessionLocal() as tx:
                ...
                execute_pickup_timeout(tx, current)
