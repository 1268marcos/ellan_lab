...
def _fetch_frame(
    ...
    q = f"""
        ...
        WHERE m.feature_date >= %s::date
          AND m.failure_label_70d IS NOT NULL
          AND m.temperature_avg_70d IS NOT NULL
          AND m.humidity_avg_70d IS NOT NULL
          AND m.battery_min_70d IS NOT NULL
          AND m.door_failures_70d IS NOT NULL
          AND m.usage_events_70d IS NOT NULL
          AND m.uptime_hours_70d IS NOT NULL
    """
...
def train(rows: list[tuple], col_names: list[str]) -> tuple[RandomForestClassifier, dict]:
    ...
    if len(np.unique(y)) < 2:
        raise RuntimeError("Target tem uma única classe; não é possível treinar/tunar métricas.")
