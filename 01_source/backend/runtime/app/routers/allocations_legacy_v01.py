# 01_source/backend_runtime/app/routers/allocations.py
# 30/03/2026
"""
Objetivo

Fazer allocations.py operar com:

locker_id resolvido por request
slots válidos vindos da topologia real do locker
allocations por locker real
commit/release coerentes com ONLINE e KIOSK

Primeiro arquivo crítico de negócio a corrigir.

Deve:

usar X-Locker-Id
validar slot pela topologia do locker
reservar/commitar/liberar no locker correto
manter idempotência por locker
...
def _default_machine_id() -> str:
    """
    Fallback legado apenas para compatibilidade.
    Em produção multi-locker, o correto é sempre usar X-Locker-Id.
    """
...
def allocate(
    ...
    try:
        ...
        # higiene mínima local deste locker
        conn.execute(
            "DELETE FROM allocations WHERE machine_id=? AND expires_at < ? AND state='EXPIRED'",
            (machine_id, _now_iso()),
        )
        ...
        # idempotência por request_id no escopo do locker
        ...
        # escolhe slot desejado ou a primeira AVAILABLE dentro do locker informado
        ...
        # claim atômico da gaveta no locker correto
        updated_at = _now_iso()
        res = conn.execute(
            """
            UPDATE door_state
        ...
        try:
            ...
            if is_unique_conflict:
                conn.execute(
                    ...
                    UPDATE allocations
                    SET state='EXPIRED'
                    WHERE machine_id=? AND door_id=? AND expires_at < ? AND state IN ('RESERVED','COMMITTED')
                    """,
                ...
                try:
                    # tenta reservar novamente a porta do mesmo locker
                    res_retry = conn.execute(
                        """
                        UPDATE door_state
                        SET state='RESERVED', product_id=?, updated_at=?
                        WHERE machine_id=? AND door_id=? AND state='AVAILABLE'
                        """,
                        (prod, _now_iso(), machine_id, door_id),
                    )
                    if res_retry.rowcount == 0:
                        _raise(
                            409,
                            err_type="RESOURCE_ALLOCATION_CONFLICT",
                            message="slot not available after cleanup",
                            retryable=True,
                            machine_id=machine_id,
                            locker_id=machine_id,
                            endpoint=str(request.url.path),
                    ...
                    st_retry = _door_state_get(conn, machine_id, door_id)
                    if st_retry == "RESERVED":
                        _door_state_upsert(conn, machine_id, door_id, "AVAILABLE", None)
                    conn.commit()
...
def commit(
    ...
    try:
        ...
        if expires_at < _now_iso():
            conn.execute(
                "UPDATE allocations SET state='EXPIRED' WHERE allocation_id=? AND machine_id=?",
                (allocation_id, machine_id),
            )
...
def release(
    ...
    try:
        ...
        current_state = _door_state_get(conn, machine_id, door_id)
        if current_state in ("RESERVED", "PAID_PENDING_PICKUP"):
            _door_state_upsert(conn, machine_id, door_id, "AVAILABLE", None)
